-- chunkname: @scripts/settings/dlcs/morris/morris_cosmetics.lua

return {
	frame_deus_portrait_khorne = {
		icon = "deus_portrait_khorne"
	},
	frame_deus_portrait_lileath_1 = {
		icon = "deus_portrait_lileath_1"
	},
	frame_deus_portrait_lileath_2 = {
		icon = "deus_portrait_lileath_2"
	},
	frame_deus_portrait_lileath_3 = {
		icon = "deus_portrait_lileath_3"
	},
	frame_deus_portrait_myrmidia_1 = {
		icon = "deus_portrait_myrmidia_1"
	},
	frame_deus_portrait_myrmidia_2 = {
		icon = "deus_portrait_myrmidia_2"
	},
	frame_deus_portrait_myrmidia_3 = {
		icon = "deus_portrait_myrmidia_3"
	},
	frame_deus_portrait_nurgle = {
		icon = "deus_portrait_nurgle"
	},
	frame_deus_portrait_sigmar_1 = {
		icon = "deus_portrait_sigmar_1"
	},
	frame_deus_portrait_sigmar_2 = {
		icon = "deus_portrait_sigmar_2"
	},
	frame_deus_portrait_sigmar_3 = {
		icon = "deus_portrait_sigmar_3"
	},
	frame_deus_portrait_slaanesh = {
		icon = "deus_portrait_slaanesh"
	},
	frame_deus_portrait_taal_1 = {
		icon = "deus_portrait_taal_1"
	},
	frame_deus_portrait_taal_2 = {
		icon = "deus_portrait_taal_2"
	},
	frame_deus_portrait_taal_3 = {
		icon = "deus_portrait_taal_3"
	},
	frame_deus_portrait_tzeentch = {
		icon = "deus_portrait_tzeentch"
	},
	frame_deus_portrait_valaya_1 = {
		icon = "deus_portrait_valaya_1"
	},
	frame_deus_portrait_valaya_2 = {
		icon = "deus_portrait_valaya_2"
	},
	frame_deus_portrait_valaya_3 = {
		icon = "deus_portrait_valaya_3"
	},
	frame_deus_portrait_citadel = {
		icon = "deus_portrait_citadel"
	},
	frame_deus_portrait_chaos_gods = {
		icon = "deus_portrait_citadel"
	},
	frame_deus_portrait_belakor = {
		icon = "deus_icon_portrait_belakor"
	}
}
